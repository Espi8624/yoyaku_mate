...
* **네트워크 오프라인 상태에 대응하는 클라이언트 견고성 확보:** 대기 중인 고객이 일시적인 음영 지역 등으로 오프라인 상태가 되더라도, 브로커 폴링(Polling) 타이머가 백그라운드에서 주기적으로 서버 상태를 모니터링하도록 설계했습니다. 이를 통해 다시 온라인으로 복구되는 즉시 점포의 승인/호출 알림 상태를 누락 없이 안전하게 화면에 동기화하는 구조를 갖췄습니다.
