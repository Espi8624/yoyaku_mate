...
* **ネットワークオフライン状態に対応するクライアント堅牢性の確保:** 待機中のお客様が一時的な通信シャドーエリアなどによりオフライン状態になっても、バックグラウンドのポーリング（Polling）タイマーが周期的にサーバー状態を監視し続けるよう設計しました。これにより、オンラインに復帰した直後、店舗側からの呼び出しや入場完了の通知を漏れなく確実に画面へ同期する堅牢な構造を構築しました。
